...
export const PPITableForm: React.FC<any> = ({ modal }: any): JSX.Element => {
  ...
  // data로 부터 url 셋팅
  useEffect(() => {
    const _url = data?.filter(item => item.url !== undefined);
  ...
  // [Event handler] URL 저장 이벤트
  const onURLSaveHandler = (_url: string) => {
    const newURL = { ...url };
    newURL.url = _url;
    setQueryData(queryClient, API_DT_PPI, mutate, url.id ? 'url/save' : 'url/add', newURL);
  };
  // 헤더에 들어갈 버튼 정의
  ...
  const PIMList = data?.filter(item => item.url === undefined);
  // 개인정보 제공 테이블
  const PPITable = <EditableTable dataSource={isLoading ? [] : data ? PIMList as any[] : []} defaultSelectOptions={defaultSelectOptions} expandKey='isForeign' headers={ppiTableHeader} innerHeaders={eppiTableHeader} isLoading={isLoading} onAdd={onAdd} onDelete={onDelete} onSave={onSave} refData={piLoading ? [] : piData} tableName={API_DT_PPI} />
  // 컴포넌트 반환
  return (
    <EditableTableForm description='‘개인정보 제3자 제공’이란, 제3자의 목적을 위해 개인정보를 외부에 제공하는 것을 말해요.\n각 제공 건에 대해 아래의 내용을 입력해주세요. 국외로 제공되는 경우에는 ‘국외 여부’에 체크한 뒤 추가 정보도 입력해야 해요.\n제3자 제공내역이 정리된 별도의 페이지가 있다면, 우측에 ‘링크(URL)’ 버튼을 클릭하여 연결시킬 수 있어요.' modal={modal} title='개인정보 제3자 제공' tools={tools}>
      {isModalOpen ? (
        <ModalToInputURL discription='제공 내용이 링크로 존재하는 경우 아래에 URL 주소를 입력해주세요.' defaultValue={url.url} open={isModalOpen} onClose={() => { setIsModalOpen(false) }} onSave={(url) => onURLSaveHandler(url)} />
      ) : (<></>)}
      {PPITable}
    </EditableTableForm>
  );
...
export const PFNITableForm: React.FC<any> = ({ modal }: any): JSX.Element => {
  ...
  const onURLSaveHandler = (_url: string) => {
    const newURL = { ...url };
    newURL.url = _url;
    setQueryData(queryClient, API_DT_PFNI, mutate, url.id ? 'url/save' : 'url/add', newURL);
  };
  // 헤더에 들어갈 버튼 정의
  const tools: JSX.Element = (
    <LinkButton onClick={() => setIsModalOpen(true)} url={url.url} />
  ...
  const PIMList = data?.filter(item => item.url === undefined);
  // 가명정보 제공 테이블
  const PFNITable = <EditableTable dataSource={isLoading ? [] : data ? PIMList as any[] : []} defaultSelectOptions={defaultSelectOptions} expandKey='isForeign' headers={pfniTableHeader} innerHeaders={epfniTableHeader} isLoading={isLoading} onAdd={onAdd} onDelete={onDelete} onSave={onSave} refData={fniLoading ? [] : fniData} tableName={API_DT_PFNI} />
  // 컴포넌트 반환
  return (
...
export const CPITableForm: React.FC<any> = ({ modal }: any): JSX.Element => {
  ...
  const onURLSaveHandler = (_url: string) => {
    const newURL = { ...url };
    newURL.url = _url;
    setQueryData(queryClient, API_DT_CPI, mutate, url.id ? 'url/save' : 'url/add', newURL);
  };
  // 헤더에 들어갈 버튼 정의
  const tools: JSX.Element = (
    <div>
  ...
  const PIMList = data?.filter(item => item.url === undefined);
  // 개인정보 위탁 테이블
  const CPITable = <EditableTable dataSource={isLoading ? [] : data ? PIMList as any[] : []} defaultSelectOptions={defaultSelectOptions} expandKey='isForeign' headers={cpiTableHeader} innerHeaders={ecpiTableHeader} isLoading={isLoading} onAdd={onAdd} onDelete={onDelete} onSave={onSave} refData={ref} tableName={API_DT_CPI} />
  // 컴포넌트 반환
  return (
    <EditableTableForm description='‘개인정보 위탁’이란 개인정보 처리 업무의 일부를 다른 업체에 맡겨 처리하는 것을 말합니다(콜센터, A/S센터, 클라우드 등).\n각 위탁 건에 대해 아래의 내용을 입력해주세요. 국외로 제공되는 경우에는 ‘국외 여부’에 체크한 뒤 추가 정보도 입력해야 해요.\n위탁 내역이 정리된 별도의 페이지가 있다면, 우측에 ‘링크(URL)’ 버튼을 클릭하여 연결시킬 수 있어요.' modal={modal} title='개인정보 위탁' tools={tools}>
      {isModalOpen ? (
        <ModalToInputURL discription='위탁 내용이 링크로 존재하는 경우 아래에 URL 주소를 입력해주세요.' defaultValue={url.url} open={isModalOpen} onClose={() => { setIsModalOpen(false) }} onSave={(url) => onURLSaveHandler(url)} />
      ) : (<></>)}
      {CPITable}
    </EditableTableForm>
  );
...
export const CFNITableForm: React.FC<any> = ({ modal }: any): JSX.Element => {
  ...
  const onURLSaveHandler = (_url: string) => {
    const newURL = { ...url };
    newURL.url = _url;
    setQueryData(queryClient, API_DT_CFNI, mutate, url.id ? 'url/save' : 'url/add', newURL);
  };
  // 헤더에 들어갈 버튼 정의
  const tools: JSX.Element = (
    <div>
  ...
  const PIMList = data?.filter(item => item.url === undefined);
  // 가명정보 위탁 테이블 
  const CFNITable = <EditableTable dataSource={isLoading ? [] : data ? PIMList as any[] : []} expandKey='isForeign' headers={cfniTableHeader} innerHeaders={ecfniTableHeader} isLoading={isLoading} onAdd={onAdd} onDelete={onDelete} onSave={onSave} refData={[]} tableName={API_DT_CFNI} />
  // 컴포넌트 반환
  return (
