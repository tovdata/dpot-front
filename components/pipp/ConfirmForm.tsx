  ...
  return (
    <>
      ...
      <Row>
        <Col span={16}>
          <DIRow>
            <Collapse activeKey={data.previous.usage ? ['1'] : []} ghost>
              <Collapse.Panel header={<DIRowHeader description='개인정보 처리방침 갱신 시, 이전 처리방침도 반드시 확인할 수 있어야 합니다.\n따라서, 본 처리방침 이전에 게재되어 있는 처리방침의 URL을 입력하여 주세요.' style={{ marginBottom: 0 }} title='플립(PLIP)에서 생성하지 않은 다른 개인정보 처리방침이 있나요?' tools={<YesOrNoRadioButton onChange={(e: any): void => onChange(sectionType, e.target.value, 'previous', 'usage')} size='small' value={data.previous.usage} />} />} key='1' showArrow={false} >
        ...
        <Col span={16}>
          <DIRow>
            <DIRowHeader description='플립에서 만든 이전 개인정보 처리방침은 새로 업데이트 되는 개인정보 처리방침에 자동으로 삽입됩니다.' title='플립 개인정보 처리방침' />
            {prevList.length > 0 ? (
              <DIRowContent>
