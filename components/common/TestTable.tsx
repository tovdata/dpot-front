...
export const EditableTable = ({ dataSource, headers, title, historyColumnInfos }: TableProps): JSX.Element => {
  return (<BaseEditableTable dataSource={dataSource} headers={headers} title={title} historyColumnInfos={historyColumnInfos} />);
}
// Component (Editable expand table)
export const EditableExpandTable = ({ dataSource, expandKey, headers, innerHeaders, title, historyColumnInfos }: ExpandTableProps): JSX.Element => {
  return (<BaseEditableTable dataSource={dataSource} expandKey={expandKey} headers={headers} innerHeaders={innerHeaders} title={title} historyColumnInfos={historyColumnInfos} />);
}

// [Internal] Component (Editable table)
const BaseEditableTable = ({ dataSource, expandKey, headers, innerHeaders, title, historyColumnInfos }: BaseEditableTableProps): JSX.Element => {
  // Set a data source
  ...
  // Create total option to be shared
  const createTotalOptions = (table: any[], columnNames: string[], defaultValue: any) => {
    let sharedOptions = {} as any;
    columnNames.forEach(column => {
      if (defaultValue[column]) {
        sharedOptions[column] = new Set<string>(defaultValue[column]);
      } else {
        sharedOptions[column] = new Set<string>();
      }
    });
    table.forEach(record => {
      columnNames.forEach(column => {
        if (record[column])
          record[column].forEach((item: any) => sharedOptions[column].add(item.name))
      })
    })
    return sharedOptions;
  }
  const totalOptions = historyColumnInfos && createTotalOptions(table, Object.keys(historyColumnInfos), historyColumnInfos);
  // Set the columns
  const columns: any[] = createEditableTableColumns(editRow, editTable, headers, false, onChangeRow, onDelete, onEditRow, onSaveRow, row, totalOptions);
  // Create a table
  const tableElement: JSX.Element = expandKey ? (
    <OuterTable columns={columns} components={{ header: { row: (props: any) => renderHeader(props, columns) } }} dataSource={table} expandable={{
      expandedRowRender: (record: any, index: number) => innerHeaders ? (<Table key={index} columns={createEditableTableColumns(editRow, editTable, innerHeaders, true, onChangeRow, onDelete, onEditRow, onSaveRow, row, totalOptions)} dataSource={(editRow && row.uuid === record.uuid) ? [row] : [record]} pagination={false} />) : (<></>),
 ...
 */
const createEditableTableColumns = (editRow: boolean, editTable: boolean, headers: TableHeadersData, isInner: boolean, onChangeRow: (column: string, display: string, value: any) => void, onDelete: (value: any) => void, onEditRow: (value: any) => void, onSaveRow: () => void, row: any, totalOptions: any): any[] => {
  ...
  const columns: any[] = Object.keys(headers).map((key: string): TableColumnProps<any> => {
    ...
    if (header.display === 'list') {
      ...
      column.render = (items: TableProcessItemData[], record: any): JSX.Element => {
        if (editRow && row.uuid === record.uuid) {
          let _totalOptions: string[] = (totalOptions && Object.keys(totalOptions).includes(key)) ? Array.from(totalOptions[key]) : [];
          if (header.required) {
            return (
              <Form.Item name={key} rules={[{ required: true }]}>
                <EditableSelectMulti defaultOptions={row[key].map((item: TableProcessItemData): string => item.name)} onChange={(items: string[]): void => onChangeRow(key, header.display, items)} totalOptions={_totalOptions} />
              </Form.Item>
            );
          } else {
            return (<EditableSelectMulti defaultOptions={row[key].map((item: TableProcessItemData): string => item.name)} onChange={(items: string[]): void => onChangeRow(key, header.display, items)} totalOptions={_totalOptions} />);
          }
        } else {
