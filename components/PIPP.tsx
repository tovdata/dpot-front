...
`;
/** [Styled Component] 개인정보 처리방침 미리보기 부분 Row */
const CDRow = styled.div`
  ...
  margin-bottom: 40px;
`;
/** [Styled Component] 개인정보 처리방침 미리보기 부분 Row content */
const CDRowContent = styled.p`
  margin-bottom: 8px;
`;
/** [Styled Component] 테이블 Form */
const StyledReadableTableForm = styled.div`
  margin-bottom: 8px;
...
`;
/** [Internal Component] 개인정보 처리방침 편집 부분 Divider */
const CIRowDivider: React.FC<any> = (): JSX.Element => {
  return (<Divider dashed style={{ marginBottom: 30, marginTop: 30 }} />);
}
/**
...
const CDRowHeader: React.FC<any> = ({ title }: any): JSX.Element => {
  return (<h2 style={{ color: '#000000', fontSize: 14, fontWeight: '600', lineHeight: '22px', marginBottom: 8 }}>◾️ {title}</h2>);
}
/**
 * [Internal Component] 개인정보 처리방침 미리보기 부분 List
 */
const CDList: React.FC<any> = ({ children, marginBottom }: any): JSX.Element => {
  return (
    <ul style={{ marginBottom: marginBottom ? marginBottom : 0, paddingLeft: 20 }}>
      {children}
    </ul>
  );
}
    ...
    <Row gutter={74} style={{ height: 'calc(100vh - 324px)' }}>
      <Col span={12} style={{ height: '100%', overflowY: 'auto' }}>
        <CIRow>
          ...
          <CIRowContent>
            <Input allowClear onChange={(e: any) => onChange(THIS_STEP, 'name', undefined, e.target.value)} placeholder='개인정보 처리자명 또는 서비스명' value={data.dInfo.name} />
          </CIRowContent>
        </CIRow>
        <CIRowDivider />
        <CIRow>
          ...
          <CIRowContent>
            <CISubject subject='관계 법령에 따른 개인정보의 보유 및 이용기간' />
            <TreeSelect showArrow={false} treeData={PeriodForPersonalInfoByRelation} treeCheckable={true} showCheckedStrategy={TreeSelect.SHOW_PARENT} onChange={(value: string[]): void => onChange(THIS_STEP, 'period', undefined, value)} placeholder='예시에서 선택' style={{ width: '100%' }} value={data.dInfo.period} />
          </CIRowContent>
        </CIRow>
        <CIRowDivider />
        <CIRow>
          <Collapse activeKey={data.dInfo.children.usage ? ['1'] : []} ghost>
            <Collapse.Panel header={<CIRowHeader style={{ marginBottom: 0 }} title='만 14세 미만 아동의 개인정보를 처리하나요?' tools={<YesOrNoRadioButton onChange={(e: any): void => onChange(THIS_STEP, 'children', 'usage', e.target.value)} size='small' value={data.dInfo.children.usage} />} />} key='1' showArrow={false}>
              <CISubject subject='법정대리인의 동의 확인 방법' />
              <TreeSelect treeData={ChildrenInfoProcessingByRelation.map((item: string): any => ({ label: item, title: item, value: item }))} onChange={(value: string[]): void => onChange(THIS_STEP, 'children', 'method', value)} treeCheckable={true} placeholder='예시에서 선택' style={{ width: '100%' }} showArrow={false} value={data.dInfo.children.method} />
            </Collapse.Panel>
          </Collapse>          
        </CIRow>
        <CIRowDivider />
        <CIRow>
          <Collapse activeKey={data.dInfo.provision.usage ? ['1'] : []} ghost>
            <Collapse.Panel header={<CIRowHeader description='해당 서비스에서 개인정보를 제3자에게 제공하는 모든 내용이 작성되어야 합니다. 수정을 원하시는 경우, "수정하기" 버튼을 눌러주세요.' title='개인정보를 제3자에게 제공하나요?' tools={<YesOrNoRadioButton onChange={(e: any): void => onChange(THIS_STEP, 'provision', 'usage', e.target.value)} size='small' value={data.dInfo.provision.usage} />} />} key='1' showArrow={false}>
        ...
        <CIRowDivider />
        <CIRow>
          <Collapse activeKey={data.dInfo.consignment.usage ? ['1'] : []} ghost>
            <Collapse.Panel header={<CIRowHeader description='해당 서비스에서 개인정보를 제3자에게 위탁하는 모든 내용이 작성되어야 합니다. 수정을 원하시는 경우, "수정하기" 버튼을 눌러주세요.' title='위탁하는 개인정보가 있나요?' tools={<YesOrNoRadioButton onChange={(e: any): void => onChange(THIS_STEP, 'consignment', 'usage', e.target.value)} size='small' value={data.dInfo.consignment.usage} />} />} key='1' showArrow={false} >
              <Button type='default' size='small' style={{ fontSize: 12 }}>수정하기</Button>
            </Collapse.Panel>
          </Collapse>
        </CIRow>
        ...
        <CIRow>
          ...
          <CIRowContent>
            <CISubject subject='개인정보를 저장하는 물리적인 공간(전산실, 자료보관실 등)이 있나요?' tools={<YesOrNoRadioButton onChange={(e: any): void => onChange(THIS_STEP, 'safety', 'physical', e.target.value)} size='small' value={data.dInfo.safety.physical} />} />
          </CIRowContent>
          <CIRowContent>
            <Collapse activeKey={data.dInfo.safety.usage ? ['1'] : []} ghost>
              <Collapse.Panel header={<CISubject subject='개인정보보호 활동을 하거나 국내외 개인정보보호 인증을 보유하고 있나요?' tools={<YesOrNoRadioButton onChange={(e: any): void => onChange(THIS_STEP, 'safety', 'usage', e.target.value)} size='small' value={data.dInfo.safety.usage} />} />} key='1' showArrow={false}>
                <Form layout='vertical'>
                  <Form.Item label='개인정보보호 활동'>
                    <Input onChange={(e: any): void => onChange(THIS_STEP, 'safety', 'activity', e.target.value)} value={data.dInfo.safety.activity} />
                  </Form.Item>
                  <Form.Item label='국내외 개인정보보호 인증 획득'>
                    <TagSelect onChange={(value: string|string[]): void => onChange(THIS_STEP, 'safety', 'certification', value)} options={['ISO/IEC 27701', 'ISMS-P', 'ABCDE']} value={data.dInfo.safety.certification} />
                  </Form.Item>
                </Form>
              </Collapse.Panel>
            </Collapse>
        ...
        <CIRowDivider />
        <CIRow>
          <CIRowHeader description='사용자의 동의 없이 추가 이용하는 경우, 그에 대한 모든 내용이 작성되어야 합니다.' title='추가적인 이용·제공 판단기준' />
        </CIRow>
        <CIRowDivider />
        <CIRow>
          <Collapse activeKey={data.dInfo.fni.usage ? ['1'] : []} ghost>
            <Collapse.Panel header={<CIRowHeader description='가명정보를 사용하는 경우, 그에 대한 모든 내용이 작성되어야 합니다. 수정을 원하시는 경우, "수정하기" 버튼을 눌러주세요.' title='가명정보를 처리하나요?' tools={<YesOrNoRadioButton onChange={(e: any): void => onChange(THIS_STEP, 'fni', 'usage', e.target.value)} size='small' value={data.dInfo.fni.usage} />} />} key='1' showArrow={false} >
              <Button type='default' size='small' style={{ fontSize: 12 }}>수정하기</Button>
            </Collapse.Panel>
          </Collapse>
        </CIRow>
        <CIRowDivider />
        <CIRow>
          <CIRowHeader title='개인정보보호책임자 및 개인정보 열람청구' tools={<Button type='default' size='small' style={{ fontSize: 12 }}>수정하기</Button>} />
        </CIRow>
        <CIRowDivider />
        <CIRow>
          <CIRowHeader description='본 내용은 필수 항목으로 반드시 들어가야합니다.' title='권익침해 구제방법' />
        </CIRow>
        <CIRowDivider />
        <CIRow>
          <CIRowHeader description='영상저보처리기기 운영에 관한 설명 영상저보처리기기 운영에 관한 설명' title='영상정보처리기기(CCTV)를 운영하나요?' tools={<YesOrNoRadioButton onChange={(e: any): void => onChange(THIS_STEP, 'fni', 'usage', e.target.value)} size='small' value={data.dInfo.fni.usage} />} />
        </CIRow>
      </Col>
...
const PreviewDocumentForPIPP: React.FC<any> = ({ data, refTable }: any): JSX.Element => {
  return (
    <>
      <h2 style={{ fontSize: 16, fontWeight: '700', lineHeight: '22px', marginBottom: 30, textAlign: 'center' }}>{data.dInfo.name} 개인정보 처리방침</h2>
      <CDRow>
        <CDRowContent>{data.dInfo.name}은(는) 정보주체의 자유와 권리 보호를 위해 「개인정보 보호법」 및 관계 법령이 정한 바를 준수하여, 적법하게 개인정보를 처리하고 안전하게 관리하고 있습니다. 이에 「개인정보 보호법」 제30조에 따라 정보주체에게 개인정보 처리에 관한 절차 및 기준을 안내하고, 이와 관련한 고충을 신속하고 원활하게 처리할 수 있도록 하기 위하여 다음과 같이 개인정보 처리방침을 수립·공개합니다.</CDRowContent>
      </CDRow>
      {data.dInfo.children.usage ? (
        <CDRow>
          <CDRowHeader title='만 14세 미만 아동의 개인정보 처리에 관한 사항' />
          <CDRowContent>
          ...
          <CDList>
            {data.dInfo.children.method.map((item: string, index: number): JSX.Element => <li key={index}>{item}</li>)}
          </CDList>
        </CDRow>
      ) : (<></>)}
      {data.dInfo.provision.usage ? (
        <CDRow>
          <CDRowHeader title='개인정보의 제3자 제공' />
          <CDRowContent>
          ...
          <ReadableTable pagination={false} columns={[
            ...
            { title: '제공 목적', dataIndex: 'purpose', key: 'purpose', render: (value: string[]) => (<ul>{value.map((item: string): JSX.Element => <li>{item}</li>)}</ul>) },
            { title: '제공 항목', dataIndex: 'items', key: 'items', render: (value: string[]) => (<>{value.join(', ')}</>) },
            { title: '보유 및 이용기간', dataIndex: 'period', key: 'period', render: (value: string[]) => (<ul>{value.map((item: string): JSX.Element => <li>{item}</li>)}</ul>) },
          ]} dataSource={refTable.provision.filter((item: any): any => !item.isForeign)} />
          <CDRowContent>
      ...
      {refTable.provision.filter((item: any): any => item.isForeign).length > 0 ? (
        <CDRow>
          <CDRowHeader title='개인정보의 제3자 제공(국외이전)' />
          <CDRowContent>
            회사는 원활한 개인정보 업무처리를 위하여 국외 법인에 아래와 같이 위탁하고 있습니다.
          </CDRowContent>
          <ReadableTable />
        </CDRow>
      ) : (<></>)}
      <CDRow>
        <CDRowHeader title='개인정보의 파기 및 절차' />
        <CDRowContent>
      ...
      {data.dInfo.consignment.usage ? (
        <CDRow>
          <CDRowHeader title='개인정보의 위탁' />
          <CDRowContent>
            ① 회사는 원활한 개인정보 업무처리를 위하여 다음과 같이 개인정보 처리업무를 위탁하고 있습니다. 보기
          </CDRowContent>
          <ReadableTable columns={[
            { title: '제공받는 자', dataIndex: 'company', key: 'company' },
            { title: '제공 목적', dataIndex: 'content', key: 'content', render: (value: string[]) => (<ul>{value.map((item: string): JSX.Element => <li>{item}</li>)}</ul>) },
            { title: '제공 항목', dataIndex: 'items', key: 'items', render: (value: string[]) => (<>{value.join(', ')}</>) },
            { title: '보유 및 이용기간', dataIndex: 'period', key: 'period', render: (value: string[]) => (<ul>{value.map((item: string): JSX.Element => <li>{item}</li>)}</ul>) },
          ]} dataSource={refTable.consignment.filter((item: any): any => !item.isForeign)} />
          <CDRowContent>
            ② 주식회사 토브데이터은(는) 위탁계약 체결 시 「개인정보 보호법」 제26조에 따라 위탁업무 수행목적 외 개인정보 처리금지, 기술적·관리적 보호조치, 재위탁 제한, 수탁자에 대한 관리·감독, 손해배상 등 책임에 관한 사항을 계약서 등 문서에 명시하고, 수탁자가 개인정보를 안전하게 처리하는지를 감독하고 있습니다.<br/>
      ...
      {data.dInfo.destructionUnused.type && data.dInfo.destructionUnused.type !== '기재안함' ? (
        <CDRow>
          <CDRowHeader title='미이용자의 개인정보 파기 등에 관한 조치'/>
      ...
      <CDRow>
        <CDRowHeader title='정보주체와 법정대리인의 권리·의무 및 행사방법' />
        <CDRowContent>
          ...
          <>
            ...
            <CDList>
              {data.dInfo.safety.activity !== '' ? (
                <li>개인정보보호 활동: {data.dInfo.safety.activity}</li>
              ) : (<></>)}
              {data.dInfo.safety.certification.length > 0 ? (
                <li>국내외 개인정보보호 인증 획득: {data.dInfo.safety.certification.join(',')}</li>
              ) : (<></>)}
            </CDList>
          </>
        ) : (<></>)}
      ...
      <CDRow>
        <CDRowHeader title='개인정보의 자동 수집 장치의 설치·운영 및 거부에 관한 사항' />
        {data.aInfo.cookie.usage ? (
          <>
            <CDRowContent>
              ① 회사는 이용자에게 개별적인 맞춤서비스를 제공하기 위해 이용정보를 저장하고 수시로 불러오는 ‘쿠키(cookie)’를 사용합니다. 쿠키는 웹사이트를 운영하는데 이용되는 서버(http)가 이용자의 컴퓨터 브라우저 또는 모바일 어플리케이션에게 보내는 소량의 정보이며 이용자들의 PC 컴퓨터내의 하드디스크에 또는 모바일 기기내에 저장되기도 합니다.<br/>
              1) 쿠키의 사용 목적: {data.aInfo.cookie.purpose.join(',')}<br/>
              2) 쿠키의 설치·운영 및 거부: {data.aInfo.cookie.disadvantage.join(', ')}<br/>
              <>
            ...
            {data.aInfo.cookie.disadvantage.length> 0 ? (
              <CDRowContent>
          ...
          <CDRowContent>① 회사는 정보주체의 이용정보를 저장하고 수시로 불러오는 ‘쿠키(cookie)’를 사용하지 않습니다.</CDRowContent>
        )}
        {data.aInfo.webLog.usage ? (
          <>
            <CDRowContent>
              ② 회사는 이용자에게 더 나은 웹사이트 이용 경험을 제공하기 위하여, 홈페이지 접속 시 자동으로 방문기록과 접속 수단에 관한 정보를 수집하여 웹사이트 트래픽을 분석하는 ‘웹 로그 분석 도구’를 사용합니다. 경우에 따라, 회사는 웹 로그 분석 업무를 타사에 위탁하며 그 과정에서 수집된 정보가 국외로 이전될 수 있습니다.<br/>
              1) 웹 로그 분석 도구의 사용 목적 : {data.aInfo.webLog.purpose.join(', ')}<br/>
              2) 웹 로그 분석 도구의 거부∙차단 방법 : {data.aInfo.webLog.method.join(', ')}<br/>
      ...
      <CDRow>
        <CDRowHeader title='행태정보의 수집·이용 및 거부 등에 관한 사항' />
        {data.aInfo.advertising.usage && data.aInfo.thirdParty.usage ? (
          <>
            ...
            <CDList>
              <li>행태정보를 수집 및 처리하려는 광고 사업자: {data.aInfo.thirdParty.company.join(', ')}</li>
              <li>형태 정보 수집 방법: {data.aInfo.thirdParty.method}</li>
              <li>수집∙처리되는 행태정보 항목: {data.aInfo.thirdParty.items.join(', ')}</li>
              <li>보유 및 이용기간: {data.aInfo.thirdParty.period}</li>
            </CDList>
            <CDRowContent>
          ...
          <CDRowContent>회사는 온라인 맞춤형 광고 등을 위한 행태정보를 수집∙이용∙제공하지 않습니다.</CDRowContent>
        )}
      </CDRow>
      <CDRow>
        <CDRowHeader title='추가적인 이용·제공 판단기준' />
        {data.aInfo.additional.usage ? (
          ...
          <CDRowContent>회사는 추가적인 이용·제공하지 않습니다.</CDRowContent>
        )}
      </CDRow>
      {data.dInfo.fni.usage ? (
        <CDRow>
          <CDRowHeader title='가명정보의 처리' />
          <CDRowContent>
            회사는 통계작성, 과학적 연구, 공익적 기록보존 등을 위하여 수집한 개인정보를 특정 개인을 알아볼 수 없도록 가명처리하여 다음과 같이 처리하고 있습니다.
          </CDRowContent>
          <CDList marginBottom={8}>
            <li>가명정보의 처리에 관한 사항</li>
          </CDList>
          <ReadableTable />
          <CDList marginBottom={8}>
            <li>가명정보의 제 3자 제공에 관한 사항</li>
          </CDList>
          <ReadableTable />
          <CDList marginBottom={8}>
            <li>가명정보 처리의 위탁에 관한 사항</li>
          </CDList>
          <ReadableTable />
          <CDRowContent>
            법 제28조의4(가명정보에 대한 안전조치 의무 등)에 따른 가면정보의 안전성 확보조치에 관한 사항<br/>
      ...
      <CDRow>
        <CDRowHeader title='개인정보보호책임자 및 개인정보 열람청구' />
        <CDRowContent>
      ...
      <CDRow>
        <CDRowHeader title='권익침해 구제 방법' />
        <CDRowContent>
          ① 회사는 정보주체의 개인정보자기결정권을 보장하고, 개인정보침해로 인한 상담 및 피해 구제를 위해 노력하고 있으며, 신고나 상담이 필요한 경우 담당부서로 연락해 주시기 바랍니다.<br/>
