 ...
 */
// export const CreatePIPPForm: React.FC<any> = ({ list, onBack, onUpdateStatus, progress, status, serviceId }: any): JSX.Element => {
//   // 현재 회사 및 서비스 조회
...
//   const { isLoading: isLoadingForData, data: loadData } = useQuery(SERVICE_PIPP, async () => await getPIPPData(serviceId));
//   // 테이블 데이터 쿼리 (API 호출)
...
//     }
//   }
//   useEffect(() => console.log('changed', data.aInfo), [data]);
//   /** [Event handler] 포커스에 따라 스코롤 이동 이벤트 (Prview part) */
//   const onFocus = (type: string, index: number, pos?: string) => { refs[type].current[index] ? refs[type].current[index].scrollIntoView((type === 'preview' && (index === 1 || index === 3 || index === 4 || index === 7)) ? { block: pos ? pos : 'start' } : { behavior: 'smooth' , block: pos ? pos : 'start' }) : undefined };
...
// export const CreatePIPP: React.FC<any> = ({ onChange, data, onFocus, onRefresh, refElements, refTable }: any): JSX.Element => {
//   const queryClient = useQueryClient();
