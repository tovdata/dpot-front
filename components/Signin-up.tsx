    ...
    <Form form={form} onFinish={onMoveStep} style={{ width: 320 }}>
      ...
      <div style={{ marginBottom: 28 }}>
        <div style={{ alignItems: 'center', display: 'flex', marginBottom: 6 }}>
          <Checkbox onChange={(e: any): void => onChange(e.target.checked, CURRENT_STEP, 'esa1')} value={data.esa1}>(필수) 서비스 이용약관 동의</Checkbox>
          <StyleLink style={{ marginLeft: 6 }}>약관 보기</StyleLink>
        </div>
        <div style={{ alignItems: 'center', display: 'flex', marginBottom: 6 }}>
          <Checkbox onChange={(e: any): void => onChange(e.target.checked, CURRENT_STEP, 'esa2')} value={data.esa2}>(필수) 개인정보 수집 및 이용 동의</Checkbox>
          <StyleLink style={{ marginLeft: 6 }}>동의서 보기</StyleLink>
        </div>
        <div style={{ alignItems: 'center', display: 'flex' }}>
          <Checkbox onChange={(e: any): void => onChange(e.target.checked, CURRENT_STEP, 'ssa1')} value={data.ssa1}>(선택) 마케팅 및 광고성 정보 수신 동의</Checkbox>
          <StyleLink style={{ marginLeft: 6 }}>동의서 보기</StyleLink>
        </div>
      </div>
      <Form.Item>
        ...
        <Row gutter={[8, 8]}>
          ...
          <Col span={19}>
            <Form.Item name='position' rules={[{ required: true, message: '직책 또는 직위를 입력해주세요.' }]} style={{ marginBottom: 0 }}>
              <Input disabled={search ? true : false} onChange={(e: any): void => onChange(e.target.value, CURRENT_STEP, 'charger', 'position')} placeholder='예) CPO, 대표이사 등' value={data.charger.position} />
            </Form.Item>
          </Col>
          ...
          <Col span={19}>
            <Form.Item name='charger' rules={[{ required: true, message: '이름을 입력해주세요.' }]} style={{ marginBottom: 0 }}>
              <Input disabled={search ? true : false} onChange={(e: any): void => onChange(e.target.value, CURRENT_STEP, 'charger', 'name')} placeholder='김OO' value={data.charger.name} />
            </Form.Item>
          </Col>
...
export const Title: React.FC<any> = ({ title }): JSX.Element => {
  return (
    <div style={{ marginBottom: 48, textAlign: 'center' }}>
      <h2 style={{ fontSize: 24, fontWeight: '500', lineHeight: '32px', margin: 0 }}>{title}</h2>
    </div>
  );
}
...
export const InputGroup: React.FC<any> = ({ children, label, required }): JSX.Element => {
  return (
    <div style={{ marginBottom: 16 }}>
      <p style={{ fontSize: 14, fontWeight: '600', lineHeight: '22px', marginBottom: 8 }}>
        {label}
        {required ? (
          <small style={{ color: '#FF4D4F', fontSize: 14, fontWeight: '600', marginLeft: 4 }}>*</small>
        ) : (<></>)}
      </p>
      {children}
    </div>
