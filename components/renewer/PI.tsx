...
export const FNITableForm: React.FC<any> = ({ accessToken, serviceId }): JSX.Element => {
  ...
  // Form tools 정의
  const tools: JSX.Element = useMemo(() => (<Button onClick={onGuide} type='default'>가명정보 입력 가이드</Button>), [onGuide]);
  // 컴포넌트 반환
  return (
...
export const PITableForm: React.FC<any> = ({ accessToken, serviceId }): JSX.Element => {
  ...
  // Form tools 정의
  const tools: JSX.Element = useMemo(() => (<Button onClick={onGuide} type='default'>현황 정보 입력 가이드</Button>), [onGuide]);
  // 컴포넌트 반환
  return (
