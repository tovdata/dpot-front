...
const Page: NextPage = ({ expand, onExpand }: any) => {
  const { isLoading, data } = useQuery('totalActivity', async () => await getActivity('service', 'b7dc6570-4be9-4710-85c1-4c3788fcbd12'));
  // 데이터 구분 및 정렬
...
const sortByDatetime = (source?: any[]): any => {
  if (source) {
    ...
    Object.keys(categories).forEach((date: string): void => {
      categories[date] = categories[date].sort((a: any, b: any): number => a.timestamp - b.timestamp);
    });
    // 반환
    return categories;
  } else {
