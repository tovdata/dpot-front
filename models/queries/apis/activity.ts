 ...
 * @returns 가공 데이터 반환
 */
export const getActivity = async (token: string, type: string, id: string): Promise<any[]> => {
  try {
    // 요청 객체 생성
 ...
 * @returns 조회 결과
 */
export const getUserActivityForWeek = async (token: string, userId: string): Promise<any[]> => {
  try {
    // 오늘 날짜
    ...
    const request: RequestDF = createRequest('GET', token);
    // API 호출
    const response = await fetch(`${SERVER_URL}activity/user/${userId}?start=${start}`, request);
    // 데이터 추출
    const result: ResponseDF = await extractData(response);
    // 데이터 반환
    return result.result ? result.data : [];
 ...
 */
export const setActivity = async (token: string, type: string, id: string, data: any): Promise<void> => {
  try {
    // 요청 객체 생성
    const request: RequestDF = createRequest('PUT', token, { text: data });
    // API 호출 및 데이터 반환
